  ...
  <BaseCard>
    <template #header>
      ...
      <div class="d-flex align-items-center flex-wrap gap-1 grid-filter">
        ...
        <div class="flex-shrink-0">
          <input v-model="filters.q" class="form-control-sm form-control" placeholder="Název, kód, #id" type="text">
        </div>
        <div class="flex-shrink-0">
          <button type="button" data-toggle="tooltip" title="" data-placement="bottom" class="btn btn-sm btn-outline-alternate disabled" data-original-title="Example Tooltip">
            <i class="fa fa-filter" /> (0)
          </button>
        </div>
        <div class="flex-shrink-0">
          <button type="button" data-toggle="tooltip" title="" data-placement="bottom" class="btn btn-sm btn-outline-dark me-2" data-original-title="Example Tooltip" @click="delete filters.q">
            <i class="fa fa-times" />
          </button>
        </div>
      </div>
    ...
    <template #body>
      <BaseGrid url="http://localhost/levior-b2b/api/producer" :page="page" :on-page="onPage" :filters="filters">
        <template #header>
          <tr>
            ...
            <th>Kód</th>
            <th>
              Název  <a style="position: relative; top: -2px;" class="dropdown-toggle btn btn-xs btn-outline-dark me-1" @click.prevent="toast.info('Editujete českou mutaci')"><span style="font-size: 10px; top: -1px; position: relative"><flag iso="gb" /></span></a>
            </th>
            <th>Pořadí</th>
            <th class="minimal"><i title="Doporučeno" class="fa fa-thumbs-o-up" /></th>
            <th class="minimal"><i title="Skryto" class="fa fa-eye-slash" /></th>
            <th class="minimal">
        ...
        <template #body="{item}">
          <tr :class="{'inactive': item.id === 5}">
            ...
            <td class="minimal">
              <button class="btn btn-xs btn-success me-1" @click="navigateTo({ name: 'producers-id', params: { id: item.uuid }})">
                <i class="fa fa-edit" />
                <slot />
              </button>
            </td>
            ...
            <td>{{ item.name.cs }} <i v-if="item.id === 3" class="fa fa-lock" /></td>
            <td class="minimal">
      ...
      <div class="d-flex align-items-center flex-wrap gap-1">
        <div class="flex-shrink-0">
          <button type="button" data-toggle="tooltip" title="" data-placement="bottom" style="padding: 4px 4px;" class="btn btn-xs btn-outline-success me-2" data-original-title="Example Tooltip" @click="delete filters.q">
            <i class="fa fa-edit" /> (10)
          </button>
        </div>
        <div class="flex-shrink-0">
          <button type="button" data-toggle="tooltip" title="" data-placement="bottom" style="padding: 4px 4px;" class="btn btn-xs btn-outline-alternate me-2" data-original-title="Example Tooltip" @click="delete filters.q">
            <i class="fa fa-download" /> (10)
          </button>
        </div>
        <div class="flex-shrink-0">
          <button type="button" data-toggle="tooltip" title="" data-placement="bottom" style="padding: 4px 4px;" class="btn btn-xs btn-outline-danger me-2" data-original-title="Example Tooltip" @click="delete filters.q">
            <i class="fa fa-trash-o" /> (10)
          </button>
        </div>
        <div class="flex-shrink-0">
