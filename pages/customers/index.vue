      ...
      <BaseGrid url="customer" :page="page" :on-page=" onPage" :filters="filters">
        ...
        <template #footer="{selectedNumber, selectAll, deleteRows, processing}">
          ...
          <div class="flex-shrink-0">
            <button @click="delete filters.q" type="button" title="" class="btn btn-xs btn-outline-success p-1 me-1" :disabled="!getSelectCount(selectAll, selectedNumber)">
              <i class="fa fa-edit"></i> ({{ getSelectCount(selectAll, selectedNumber) }})
            </button>
          </div>
          <div class="flex-shrink-0">
            <button @click="delete filters.q" type="button" title="" style="padding: 4px 4px;" class="btn btn-xs btn-outline-alternate me-1" :disabled="!getSelectCount(selectAll, selectedNumber)">
          ...
          <BaseGridPaginator wrap="flex-shrink-0" ref="paginator" :url="'customer'" :page="page" :on-page="onPage" :filters="filters" @change-page="page = $event" @change-on-page="onPage = $event; page = 1;"/>
        </template>
      </BaseGrid>
    </template>
  </BaseCard>
