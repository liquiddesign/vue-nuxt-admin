  ...
  <ul class="vertical-nav-menu metismenu">
    <template v-for="(header, index) in menu" :key="index">
      ...
      <li v-for="(item, index2) in header.items" :key="index2" :class="{'mm-active' : active[item.name] === true || item.route === route.name}">
        <a v-if="!item.hidden" href="#" :aria-expanded="active[item.name] === true ? 'true' : 'false'" class="ps-0" @click.stop.prevent="item.items.length ? active[item.name] = !active[item.name] : navigateTo({name: item.route})">
          <div class="d-flex align-items-center gap-3">
            <BaseIcon :icon-name="item.icon" :size="24" :stroke-width="1" class="metismenu-icon text-black-50" />
            {{ item.title }}
