<template>
  {{ UpdateStrategy }}| {{ UpdateStrategyLabels }}
      ...
      <BaseGridPaginator v-slot="{ totalCount }" wrap="flex-shrink-0" :url="url" :page="page" :on-page="onPage" :filters="filters" @change-page="page = $event" @change-on-page="onPage = $event; page = 1;">
        ...
        <BaseModal ref="modalUpdate" :title="'Hromadná úprava (celkem ' + selectedCount(totalCount) + ')'">
          ...
          <template #footer="{close}">
            <BaseButton class="btn-primary" :loading="form?.pending" wrap="flex-shrink-0" @click="$refs.form?.submit(); close();">Upravit záznamy</BaseButton>
          </template>
        </BaseModal>
      </BaseGridPaginator>
    </template>
...
import {UpdateStrategyLabels} from '../../utils/UpdateStrategy';
...
const url: string = 'pricelist';
const form = ref<InstanceType<typeof BaseForm>>();

const defaultFormData: any = {recommended: {strategy: 'noAction', value: false}, hidden: {strategy: 'noAction', value: false}};
